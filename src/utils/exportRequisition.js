    ...
    const exportToPDF = () => {
        import('jspdf').then(jsPDF => {
            import('jspdf-autotable').then(async () => {
                ...
                doc.text(`Date: ${customDateFormat(requisitionDetails?.createdAt).split(",")[0]}`, 30, 75);
                doc.text(`Department: ${requisitionDetails?.department}`, 30, 85);

                doc.text(`Proposed amount: ${requisitionDetails?.proposedAmount}`, 190, 55);
                doc.text(`#Proposed items: ${requisitionDetails?.totalProposedItems}`, 190, 65);
                doc.text(`Purchase amount: ${requisitionDetails?.purchasedAmount || "__________"}`, 190, 75);
                doc.text(`#Purchase items: ${requisitionDetails?.purchasedItems || "__________"}`, 190, 85);
...
exports.exportRequisitionReport = (requisitionDetails) => {
    ...
    const customizedRequisitionDetails = requisitionDetails.requisitions.map((requisition) => {
        const customizedDate = customDateFormat(requisition.createdAt).split(",")[0];
    ...
    const exportToPDF = () => {
        import('jspdf').then(jsPDF => {
            import('jspdf-autotable').then(async () => {
                ...
                doc.text(`Report Generated: ${customDateFormat(new Date()).split(",")[0]}`, 30, 75);
                doc.text(`Generated By: ${requisitionDetails?.generatedBy}`, 30, 85);

                doc.text(`Required Amount: ${requisitionDetails?.totalProposedAmount}`, 190, 55);
                doc.text(`Purchased Amount: ${requisitionDetails?.totalPurchasedAmount}`, 190, 65);
                doc.text(`Required Items: ${requisitionDetails?.totalProposedItems}`, 190, 75);
                doc.text(`Purchased Items: ${requisitionDetails?.totalPurchasedItems}`, 190, 85);
                ...
                doc.autoTable(exportColumns, customizedRequisitionDetails.sort(), {
                    startY: 95,
