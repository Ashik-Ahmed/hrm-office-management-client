...
export async function getConveyanceDetailsByEmployeeEmail(email, month, year, accessToken) {
    const url = `${process.env.NEXT_PUBLIC_API_SERVER_URL}/conveyance/${email}?month=${month}&year=${year}`;
...
export async function getConveyanceByEmployeeEmail(employeeEmail) {
    ...
    const conveyance = await fetch(`${process.env.NEXT_PUBLIC_API_SERVER_URL}/conveyance/${employeeEmail}?month=${month}&year=${year}`,
        {
            next: { revalidate: 3 }
        }
    )
...
export async function getAllEmployeeMonthlyConveyance() {
    const month = parseInt(new Date().getMonth() + 1)
    const year = parseInt(new Date().getFullYear())
    const allConveyance = await fetch(`${process.env.NEXT_PUBLIC_API_SERVER_URL}/conveyance/?month=${month}&year=${year}`, {
        next: { revalidate: 3 }
    })
        .then(res => res.json())
